...
@Table(name = "tag")
@NamedQueries({
		@NamedQuery(name = "Tag.findAll", query = "SELECT DISTINCT t FROM Tag t"),
		@NamedQuery(name = "Tag.findByName", query = "SELECT t FROM Tag t WHERE t.name = :name"),
		@NamedQuery(name = "Tag.findById", query = "SELECT DISTINCT t FROM Tag t LEFT JOIN FETCH t.papers WHERE t.id = :id")
})
